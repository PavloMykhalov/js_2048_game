...
function updateBoard() {
    gameBoard.forEach((row, rowIndex)=>{
        row.forEach((value, columnIndex)=>{
            ...
            cell.className = value === 0 ? "field-cell" : `field-cell field-cell--${value}`;
        });
    });
...
function checkWin() {
    for(let row = 0; row < BOARD_SIZE; row++){
        for(let col = 0; col < BOARD_SIZE; col++)if (gameBoard[row][col] === 2048) {
...
function moveTilesUp() {
    for(let colIndex = 0; colIndex < BOARD_SIZE; colIndex++){
        let column = [];
        for(let rowIndex = 0; rowIndex < BOARD_SIZE; rowIndex++)column.push(gameBoard[rowIndex][colIndex]);
        column = slideTiles(column);
        for(let rowIndex = 0; rowIndex < BOARD_SIZE; rowIndex++)gameBoard[rowIndex][colIndex] = column[rowIndex];
    }
}
function moveTilesDown() {
    for(let colIndex = 0; colIndex < BOARD_SIZE; colIndex++){
        let column = [];
        for(let rowIndex = 3; rowIndex >= 0; rowIndex--)column.push(gameBoard[rowIndex][colIndex]);
        column = slideTiles(column);
        for(let rowIndex = 3; rowIndex >= 0; rowIndex--)gameBoard[rowIndex][colIndex] = column.shift();
    }
}
